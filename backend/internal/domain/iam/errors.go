...
var (
	...
	// ErrSessionIdle ends a console login that has not been used for longer than
	// the idle timeout — which is what a closed browser looks like from here.
	ErrSessionIdle = errors.New("iam: signed out after inactivity")
	// ErrSessionLifetime ends a console login that has reached its absolute
	// lifetime, however actively it was being used.
	ErrSessionLifetime     = errors.New("iam: session reached its maximum length")
