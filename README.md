...
```bash
# Both report the version this deployment is running — it should match the
# VERSION file, or whatever you set GUARDRAIL_VERSION to.
curl -fskS https://localhost/api/v1/version   # {"name":"GuardRail","version":"..."}
curl -fskS https://localhost/healthz          # {"status":"ok","version":"..."}
```
...
```bash
# an A record. Reloaded live — takes effect in about a second, no restart.
cat >> deploy/dns/hosts/estate <<'EOF'
10.200.10.98   fw1.corp.lan fw1
10.200.10.99   switch1.corp.lan
EOF

# wildcards, CNAME, SRV, TXT. Read only at startup.
echo 'address=/lab.corp.lan/10.200.10.97' > deploy/dns/conf.d/lab.conf
docker compose --profile dns restart dns
```

dnsmasq watches `hosts/` with inotify but never re-reads its own configuration,
which is why one reloads and the other needs a restart. Records are gitignored —
they name your estate. Full notes in **[deploy/dns/README.md](deploy/dns/README.md)**.
...
- **Data plane**: PostgreSQL 16 (with Row-Level Security), Redis 7 — both
...
│   ├── migrations/           # golang-migrate SQL (0001..0036)
...
├── deploy/
│   ├── dns/                  # your own DNS records: hosts/ (live) + conf.d/ (restart)
│   ├── guacd/tls/            # guacd's certificate, generated by install.sh
│   ├── postgres/init/        # app-role bootstrap for RLS
│   ├── postgres/hba/         # client auth — hostssl, so TLS is required not offered
│   ├── postgres/tls/         # database certificate, generated by install.sh
│   ├── siem/                 # pinned SIEM certificate for SSO, if used
│   └── traefik/dynamic/      # Traefik routes + TLS config
├── scripts/                  # install.sh, bootstrap.sh, run-host-api.sh, siem-sso.sh, release.sh
├── docker-compose.yml        # the whole stack: postgres, redis, guacd, migrate, seed, api, web, traefik, dns
├── Makefile
├── .env.example
